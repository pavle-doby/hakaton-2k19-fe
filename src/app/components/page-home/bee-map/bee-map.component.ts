...
export class BeeMapComponent implements OnInit {
  ...
  constructor(
    private beeMapService: BeemapService,
  ...
  getAllBeeHives(box: any) {
    this.beeMapService.getAllBeeHives(box).subscribe(res => {
      ...
      temp.forEach(bee => {
        this.beeHives.push({
          pk: bee.pk,
          coordinate: { lat: bee.latitude, lng: bee.longitude },
          name: bee.name,
          count: bee.hive_count
        });
      });
    ...
    this.beeMapService.getAllDangerZones(box).subscribe(res => {
      ...
      temp.forEach(zone => {
        this.zones.push({
          pk: zone.pk,
          upperLeftPoint: {
            lat: zone.upper_left_latitude,
            lng: zone.upper_left_longitude
          },
          bottomRightPoint: {
            lat: zone.bottom_right_latitude,
            lng: zone.bottom_right_longitude
          },
          dateTimeEnd: null,
          dateTimeStart: null
        });
  ...
  markerDragEnd($event: any, bee: BeeHives) {
    let beeToUpdate;
    this.beeHives[
      this.beeHives.findIndex(el => el.pk === bee.pk)
    ].coordinate.lat = $event.coords.lat;
    this.beeHives[
      this.beeHives.findIndex(el => el.pk === bee.pk)
    ].coordinate.lng = $event.coords.lng;
    this.beeMapService
      .updateBeeHive(
        this.beeHives[this.beeHives.findIndex(el => el.pk === bee.pk)]
      )
      .subscribe(res => {
        // console.log(res);
      });
